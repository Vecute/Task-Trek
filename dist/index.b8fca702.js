...
function moveCard(card, newStatus) {
    ...
    if (newStatus === "todo" && !card.classList.contains("todo__card")) newContainer = (0, _mainJs.todoContainer); // Если новый статус 'todo', назначаем контейнер todoContainer
    else if (newStatus === "progress" && !card.classList.contains("progress__card")) newContainer = (0, _mainJs.progressContainer); // Если новый статус 'progress', назначаем контейнер progressContainer 
    else if (newStatus === "completed" && !card.classList.contains("completed__card")) newContainer = (0, _mainJs.completedContainer); // Если новый статус 'completed', назначаем контейнер completedContainer
    else return; // Завершаем функцию, если перемещение происходит в свой же контейнер
